...
class SeqNN():
    ...
    def build_block(self, current, block_params):
        ...
        global_vars = ['activation', 'batch_norm', 'bn_momentum', 'bn_type',
                       'l2_scale', 'l1_scale', 'padding', 'activation_end']
    ...
    def predict_on_batch(self, seqs, head_i=None):
        """ Predict on a single batch of sequences. """
        # choose model
        if self.ensemble is not None:
            model = self.ensemble
        elif head_i is not None:
        ...
        return model(seqs, training=False)
