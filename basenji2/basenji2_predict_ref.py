...
def main():
    """
    Use Basenji2 to predict expression for all genes using the reference genome.
    ...
    parser.add_option("--n_uniform", dest="n_uniform",
                      default=False,
                      action="store_true",
                      help="Replace Ns with 0.25s rather than randomly picking a nucleotide [Default: %default]")
    ...
    # Read genes file
    genes = natsorted([os.path.basename(file) for file in glob.glob(f"{consensus_dir}/*")])  # we run predictions in natural gene sort order
    ...
    preds_stream = stream.PredStreamGen(seqnn_model,
                                        gen_ref_seqs_for_all_genes(consensus_dir,
                                                                   genes,
    ...
    ref_gene_preds = []
    for gi, _ in enumerate(tqdm(genes)):
...
def get_1_seq_from_fasta(fasta_file: str) -> str:
    ...
    Also deals with possible seq truncations (if we don't have enough context for ENFORMER_SEQ_LENGTH) from beginning
    or end of chromosome by padding appropriately with Ns.

    Returns:
    - seq: uppercase sequence from fasta file
    """
...
def gen_ref_seqs_for_all_genes(consensus_dir: str,
                               genes: List[str],
                               seq_length: int,
    ...
    - n_uniform: if True, replace Ns with 0.25 for uniform background
    """
    for gene in genes:
